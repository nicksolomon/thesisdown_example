...
```{r}
nice_eq <- function(eqn = "y = mx + b", crossref = "line"){
        cat(paste0("\n\\begin{equation}\n",
          eqn,
          "(\\#eq:", crossref, ")",
          "\\end{equation}\\noindent"
...
nice_eq(eqn = "P(\\mathbf{Y} = \\mathbf{y}) = \\frac{e^{\\theta \\cdot g(\\mathbf{y})}}{C(\\theta, \\mathcal{Y})}", crossref = "ergm")
```
That works, but not very easily.
